...
void AGIDL_TIMSetWidth(AGIDL_TIM *tim, const int width){
	tim->img_header.width = width;
}

void AGIDL_TIMSetHeight(AGIDL_TIM *tim, const int height){
	tim->img_header.height = height;
}

void AGIDL_TIMSetClrFmt(AGIDL_TIM *tim, const AGIDL_CLR_FMT fmt){
	tim->fmt = fmt;
}

void AGIDL_TIMSetClr(const AGIDL_TIM *tim, const int x, const int y, const COLOR clr){
...
void AGIDL_TIMSetClr16(const AGIDL_TIM *tim, const int x, const int y, const COLOR16 clr){
	if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim) == 16)){
...
void AGIDL_TIMSetRGB(const AGIDL_TIM *tim, const int x, const int y, const u8 r, const u8 g, const u8 b){
...
void AGIDL_TIMSetICPMode(AGIDL_TIM *tim, const int mode){
	tim->icp = mode;
}

void AGIDL_TIMSetICPEncoding(AGIDL_TIM* tim, const AGIDL_ICP_ENCODE encode){
	tim->encode = encode;
}

void AGIDL_TIMSetMaxDiff(AGIDL_TIM* tim, const int max_diff){
	tim->max_diff = max_diff;
}

void AGIDL_TIMForce4BPPEncoding(AGIDL_TIM* tim, const AGIDL_Bool force4bpp){
	tim->force4bpp = force4bpp;
}

void AGIDL_TIMInitAttributes(AGIDL_TIM* tim, const u16 pal_mem_add_x, const u16 pal_mem_add_y, const u16 img_mem_add_x, const u16 img_mem_add_y){
...
void AGIDL_ClearTIM(const AGIDL_TIM *tim, const COLOR clr){
	if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) != 16){
...
void AGIDL_ClearTIM16(const AGIDL_TIM *tim, const COLOR16 clr){
	if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) == 16){
		AGIDL_ClrMemset16(tim->pixels.pix16,clr,AGIDL_TIMGetSize(tim));
	}
	else{
...
void AGIDL_ClearColorTIM(const AGIDL_TIM* tim, const float r, const float g, const float b){
	if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) == 16){
...
COLOR AGIDL_TIMGetClr(const AGIDL_TIM *tim, const int x, const int y){
...
COLOR16 AGIDL_TIMGetClr16(const AGIDL_TIM *tim, const int x, const int y){
	if(x >= 0 && y >= 0 && x < AGIDL_TIMGetWidth(tim) && y < AGIDL_TIMGetHeight(tim)){
...
void AGIDL_ColorConvertTIM(AGIDL_TIM* tim, const AGIDL_CLR_FMT dest){
	const u8 sbits = AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)), dbits = AGIDL_GetBitCount(dest);
...
void AGIDL_TIMCopyPix(const AGIDL_TIM* tim, const COLOR* clrs, const u32 count){
	if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) != 16){
		AGIDL_ClrMemcpy(tim->pixels.pix32,clrs,count);
	}
}

void AGIDL_TIMCopyPix16(const AGIDL_TIM* tim, const COLOR16* clrs, const u32 count){
	if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) == 16){
		AGIDL_ClrMemcpy16(tim->pixels.pix16,clrs,count);
	}
}

AGIDL_TIM * AGIDL_CreateTIM(const char *filename, const int width, const int height, const AGIDL_CLR_FMT fmt){
	...
	if(tim->header.version == TIM_4BPP){
		...
		if(tim->clut_header.num_clrs == 16 || tim->clut_header.num_icps == 16){
			...
			for(int y = 0; y < AGIDL_TIMGetHeight(tim); y++){
				for(int x = 0; x < AGIDL_TIMGetWidth(tim); x+=4){
					...
					const COLOR16 clr  = tim->palette.icp.palette_16b_16[index1];
		...
		if(tim->clut_header.num_clrs == 256 || tim->clut_header.num_icps == 256){
			...
			for(i = 0; i < AGIDL_TIMGetSize(tim); i++){
				...
				const COLOR16 clr = tim->palette.icp.palette_16b_256[index];
		...
		for(int y = 0; y < AGIDL_TIMGetHeight(tim); y++){
			for(int x = 0; x < AGIDL_TIMGetWidth(tim); x += 2){
				const u16 pix1 = AGIDL_ReadShort(file);
...
void AGIDL_TIMEncodeICP(AGIDL_TIM* tim){
	if(tim->encode == ICP_ENCODE_THRESHOLD){
		...
		for(int y = 0; y < AGIDL_TIMGetHeight(tim); y++){
			for(int x = 0; x < AGIDL_TIMGetWidth(tim); x++){
				COLOR16 clr16;
				if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) == 32){
					const COLOR clr = AGIDL_TIMGetClr(tim,x,y);
...
void AGIDL_TIMEncodeNearestICP(const AGIDL_TIM* tim, const AGIDL_ICP palette, FILE* file){
	const AGIDL_CLR_FMT palfmt = palette.fmt;
	...
	if(AGIDL_GetBitCount(palfmt) == 16 && AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) == 16){
		...
		for(int i = 0; i < AGIDL_TIMGetSize(tim); i++){
			const COLOR16 clr = tim->pixels.pix16[i];
			const u8 index = AGIDL_FindNearestColor(palette,clr,AGIDL_TIMGetClrFmt(tim));
...
void AGIDL_TIMEncodeIMG(const AGIDL_TIM* tim, FILE* file){
	if(tim->icp == TRUE){
		if(tim->force4bpp != TRUE){
			...
			const float scale = (float)w/AGIDL_TIMGetWidth(tim);
			...
			for(int y = 0; y < AGIDL_TIMGetHeight(tim); y++){
				for(int x = 0; x < w; x += 4){
					...
					const COLOR16 clr3 = AGIDL_TIMGetClr16(tim,x2+2,y);
					const COLOR16 clr4 = AGIDL_TIMGetClr16(tim,x2+3,y);

					const u8 index1 = AGIDL_FindNearestColor(fourbpp,clr1,AGIDL_TIMGetClrFmt(tim));
					const u8 index2 = AGIDL_FindNearestColor(fourbpp,clr2,AGIDL_TIMGetClrFmt(tim));
					const u8 index3 = AGIDL_FindNearestColor(fourbpp,clr3,AGIDL_TIMGetClrFmt(tim));
					const u8 index4 = AGIDL_FindNearestColor(fourbpp,clr4,AGIDL_TIMGetClrFmt(tim));
...
AGIDL_TIM * AGIDL_LoadTIM(char *filename){
	...
	const int error1 = AGIDL_TIMDecodeHeader(tim,file);
	const int error2 = AGIDL_TIMDecodeIMG(tim,file);
...
void AGIDL_ExportTIM(AGIDL_TIM *tim){
	...
	if(tim->icp == TRUE){
		if(AGIDL_GetBitCount(AGIDL_TIMGetClrFmt(tim)) == 32){
			...
			const AGIDL_CLR_FMT src = AGIDL_TIMGetClrFmt(tim);
