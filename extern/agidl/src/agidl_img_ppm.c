...
void AGIDL_PPMSetWidth(AGIDL_PPM* ppm, const int width){
	ppm->width = width;
}

void AGIDL_PPMSetHeight(AGIDL_PPM* ppm, const int height){
	ppm->height = height;
}

void AGIDL_PPMSetClrFmt(AGIDL_PPM* ppm, const AGIDL_CLR_FMT fmt){
	ppm->fmt = fmt;
}

void AGIDL_PPMSetType(AGIDL_PPM* ppm, const AGIDL_PPM_TYPE type){
	ppm->type = type;
}

void AGIDL_PPMSetMaxDiff(AGIDL_PPM* ppm, const int max_diff){
	ppm->max_diff = max_diff;
}

void AGIDL_PPMSetClr(const AGIDL_PPM* ppm, const int x, const int y, const COLOR clr){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
		ppm->pixels.pix32[x+y*AGIDL_PPMGetWidth(ppm)] = clr;
	}
}

void AGIDL_PPMSetClr16(const AGIDL_PPM* ppm, const int x, const int y, const COLOR16 clr){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
		ppm->pixels.pix16[x+y*AGIDL_PPMGetWidth(ppm)] = clr;
	}
}

void AGIDL_PPMSetRGB(const AGIDL_PPM* ppm, const int x, const int y, const u8 r, const u8 g, const u8 b){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
...
void AGIDL_PPMSetRGBA(const AGIDL_PPM* ppm, const int x, const int y, const u8 r, const u8 g, const u8 b, const u8 a){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
...
void AGIDL_PPMSetColor3f(const AGIDL_PPM* ppm, const int x, const int y, const f32 r, const f32 g, const f32 b){
if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
	const int bits = AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm));
		if(bits == 24 || bits == 32){
...
void AGIDL_PPMSetColor4f(const AGIDL_PPM* ppm, const int x, const int y, const f32 r, const f32 g, const f32 b, const f32 a){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
		if(AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)) == 32){
...
COLOR AGIDL_PPMGetClr(const AGIDL_PPM* ppm, const int x, const int y){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
...
COLOR16 AGIDL_PPMGetClr16(const AGIDL_PPM* ppm, const int x, const int y){
	if(x >= 0 && y >= 0 && x < AGIDL_PPMGetWidth(ppm) && y < AGIDL_PPMGetHeight(ppm)){
...
void AGIDL_ClearPPM(const AGIDL_PPM *ppm, const COLOR clr){
...
void AGIDL_ClearPPM16(const AGIDL_PPM *ppm, const COLOR16 clr){
	if(AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)) == 16){
		AGIDL_ClrMemset16(ppm->pixels.pix16,clr,AGIDL_PPMGetSize(ppm));
	}
	else{
...
void AGIDL_ClearColorPPM(const AGIDL_PPM* ppm, const float r, const float g, const float b){
	if(AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)) == 16){
...
void AGIDL_PPMCopyPix(const AGIDL_PPM* ppm, const COLOR* clrs, const u32 count){
	if(AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)) != 16){
		AGIDL_ClrMemcpy(ppm->pixels.pix32,clrs,count);
	}
}

void AGIDL_PPMCopyPix16(const AGIDL_PPM* ppm, const COLOR16* clrs, const u32 count){
	if(AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)) == 16){
...
void AGIDL_ColorConvertPPM(AGIDL_PPM* ppm, const AGIDL_CLR_FMT dest){
	const u8 sbits = AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)), dbits = AGIDL_GetBitCount(dest);
...
int AGIDL_PPMDecodeHeader(AGIDL_PPM* ppm, FILE* file){
	const u8 byte1 = AGIDL_ReadByte(file), byte2 = AGIDL_ReadByte(file);
...
AGIDL_PPM_TYPE AGIDL_PPMGetType(const u8 byte1, const u8 byte2){
	if(byte1 == 'P' && byte2 == '6'){
		return PPM_BIN;
	}
	if(byte1 == 'P' && byte2 == '3'){
...
u8 AGIDL_PPMGetNum(const char c){
	switch(c){
		case '0':{
			return 0;
		}
...
void AGIDL_PPMDecodeIMG(AGIDL_PPM* ppm, FILE* file){
	if(ppm->type == PPM_BIN){
		...
		const float scale = 255.0f/ppm->bpp;

		for(int i = 0; i < AGIDL_PPMGetSize(ppm); i++){
			const u8 r = AGIDL_ReadByte(file)*scale;
			const u8 g = AGIDL_ReadByte(file)*scale;
			const u8 b = AGIDL_ReadByte(file)*scale;
...
AGIDL_PPM* AGIDL_LoadPPM(const char* filename){
	...
	const int error = AGIDL_PPMDecodeHeader(ppm,file);
...
AGIDL_PPM* AGIDL_CreatePPM(const char* filename, const int width, const int height, const AGIDL_CLR_FMT fmt){
	AGIDL_PPM* ppm = malloc(sizeof(AGIDL_PPM));
...
void AGIDL_PPMEncodeHeader(const AGIDL_PPM* ppm, FILE* file){
	...
	const u32 w = AGIDL_PPMGetWidth(ppm);
	const u32 h = AGIDL_PPMGetHeight(ppm);
	const u8 bpp = 255;
...
void AGIDL_PPMEncodeIMG(const AGIDL_PPM* ppm, FILE* file){
	if(AGIDL_GetBitCount(AGIDL_PPMGetClrFmt(ppm)) != 16){
		if(ppm->type == PPM_BIN){
			const AGIDL_CLR_FMT fmt = AGIDL_PPMGetClrFmt(ppm);

			for(int i = 0; i < AGIDL_PPMGetSize(ppm); i++){
				const u32 color = ppm->pixels.pix32[i];
