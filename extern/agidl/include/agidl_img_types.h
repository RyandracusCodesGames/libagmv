...
void AGIDL_ClrMemcpy(COLOR *dest, const COLOR *src, u32 count);
void AGIDL_ClrMemcpy16(COLOR16 *dest, const COLOR16 *src, u32 count);
int AGIDL_CheckCoordBounds(u32 x, u32 y, u32 width, u32 height);
void AGIDL_CopyTile(void* dest, const void* src, u32 destw, u32 desth, u32 srcw, u32 srch, AGIDL_CLR_FMT destfmt, AGIDL_CLR_FMT srcfmt, u16 xstart, u16 xend, u16 ystart, u16 yend, u16 dx, u16 dy);
void AGIDL_CopyScanline(void* dest, const void* src, u32 destw, u32 desth, u32 srcw, u32 srch, AGIDL_CLR_FMT destfmt, AGIDL_CLR_FMT srcfmt, u16 destscanline, u16 srcscanline);
...
void AGIDL_ExportRawColors(const void* data, u32 width, u32 height, AGIDL_CLR_FMT fmt, AGIDL_FILE_TYPE ftype, AGIDL_ARR_TYPE arrtype, u8 rgb);
u8 * AGIDL_GenerateImgDataFromICP(const void* data, u32 width, u32 height, AGIDL_ICP icp, int max_diff);
COLOR AGIDL_GetClr(const COLOR* clrs, int x, int y, int width, int height);
COLOR16 AGIDL_GetClr16(const COLOR16* clrs, int x, int y, int width, int height);
...
void AGIDL_24BPPTO16BPP(const COLOR* src, COLOR16* dest, int width, int height, AGIDL_CLR_FMT *fmt);
void AGIDL_16BPPTO24BPP(const COLOR16* src, COLOR* dest, int width, int height, AGIDL_CLR_FMT *fmt);
...
u8* AGIDL_GenerateRGBBuffer(const void* data, int width, int height, AGIDL_CLR_FMT fmt);
u8* AGIDL_GenerateRGBABuffer(const COLOR* src, int width, int height, AGIDL_CLR_FMT fmt);
u8* AGIDL_GenerateBGRBuffer(const COLOR* src, int width, int height, AGIDL_CLR_FMT fmt);
COLOR* AGIDL_RGBSyncClrs(u8* rgbbuff, int width, int height, AGIDL_CLR_FMT fmt);
COLOR* AGIDL_RGBASyncClrs(const u8* rgbbuf, int width, int height, AGIDL_CLR_FMT fmt);
void AGIDL_FreeClrs(COLOR* clrs);
void AGIDL_FreeClrs16(COLOR16* clrs);
void AGIDL_FreeRGBBuffer(u8* rgbbuf);
void AGIDL_PrintRGBBuffer(const u8* rgbbuf, int width, int height, AGIDL_CLR_FMT fmt);
void AGIDL_PrintRGBABuffer(const u8* rgbbuf, int width, int height, AGIDL_CLR_FMT fmt);
#endif
